...
let btndec = document.querySelector(".descending");
let btname = document.querySelector(".name");
let btasc = document.querySelector("asc")
...
search.addEventListener("keyup", function (event) {
  let filtered = users.filter((item) => {
    return item.first_name.toLocaleLowerCase().includes(event.target.value);
  });
...
btnasc.addEventListener("click", function () {
  let userasc = users.sort((a, b) => a.id - b.id);
  drawTable(userasc);
  btnasc
});

btndec.addEventListener("click", function () {
  let userdec = users.sort((a, b) => b.id - a.id);
  drawTable(userdec);
});

btname.addEventListener("click", function () {
  let username = users.sort((a, b) => a.first_name.localeCompare(b.first_name));
  drawTable(username);
});
